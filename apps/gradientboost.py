...
def display_data(df):
    ...
    session_state = SessionState.get(checkboxed=False,my_value=pd.DataFrame(),button=False,model=GradientBoostingRegressor(),proc_df=pd.DataFrame(),cat_var=[],y=pd.DataFrame())
    ...
    if (st.sidebar.button('Next') or session_state.checkboxed):
        ...
        if (st.button('Fit Model')):
            ...
            if cats:
                ...
                print(categorical_features_indices)
                model,df_returned=gradientBoostPredict(X_train,y_train,n_estimators,max_depth,categorical_features_indices)
                ...
                print(X.shape,y.shape)
                model,df_returned=gradientBoostPredict(X_train,y_train,n_estimators,max_depth)
                params = model.get_params()
                #for key,value in params.items():
                    #st.write("%s : %s" %(key, value))
                param_df=pd.DataFrame(params.items(), columns=['Parameter', 'Value'])
